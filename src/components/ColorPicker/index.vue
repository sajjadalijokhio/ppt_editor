...
// Each time you select a color that is not a preset, you need to add that color to the recent use list
...
// Open the color picker pipette
// Check whether the environment supports native color picker eyedroppers, use native eyedroppers 
// if they do, otherwise use custom eyedroppers
...
// Native color picker straw
const browserEyeDropper = () => {
  message.success('press ESC key to close the color picker pipette', { duration: 0 })
...
// Custom Canvas-based color picker eyedropper
const customEyeDropper = () => {
  ...
  toCanvas(targetRef, { filter, fontEmbedCSS: '', width, height, canvasWidth: width, canvasHeight: height, pixelRatio: 1 }).then(canvasRef => {
    ...
    message.error('Failed to initialize the color-picking pipette')
    document.body.removeChild(maskRef)
  })
}
</script>
