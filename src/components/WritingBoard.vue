...
// Mouse Position Coordinates: Used for brush or eraser position following
const mouse = ref({
  x: 0,
  y: 0,
})

// Whether the mouse is within canvas range: The brush or eraser will not appear until it is within range
...
// When switching brush mode, update the Canvas CTX configuration
const updateCtx = () => {
  if (!ctx) return
  if (props.model === 'mark') {
...
// The drawing speed is calculated based on the distance s and time t between the two mouse movements, the faster the speed, the finer the ink
...
// Get the relative position of the mouse in the canvas
...
// Ready to start drawing/erasing ink (pen)
...
// End Drawing/Erase Ink (Stop Pen)
const handleMouseup = () => {
