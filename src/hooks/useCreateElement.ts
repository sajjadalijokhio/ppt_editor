...
interface LineElementPosition extends CommonElementPosition {
  start: [number, number]
  end: [number, number]
}
...
export default () => {
  ...
  // Creates (inserts) an element and sets it as the selected element
  ...
  const createTextElement = (position: CommonElementPosition, data?: { content?: string; vertical?: boolean }) => {
  ...
  const createShapeElement = (position: CommonElementPosition, data: PPTShapeElement, supplement: Partial<PPTShapeElement> = {}) => {
    const { left, top, width, height } = position
  ...
  const createLineElement = (position: LineElementPosition, data: PPTLineElement) => {
    ...
    if (data.broken) newElement.broken = [(start[0] + end[0]) / 2, (start[1] + end[1]) / 2]
    if (data.curve) newElement.curve = [(start[0] + end[0]) / 2, (start[1] + end[1]) / 2]
    if (data.cubic) newElement.cubic = [[(start[0] + end[0]) / 2, (start[1] + end[1]) / 2], [(start[0] + end[0]) / 2, (start[1] + end[1]) / 2]]
    createElement(newElement)
  }
