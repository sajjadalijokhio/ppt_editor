<template>
  <div class="canvas-tool">
    <div class="left-handler">
      <IconBack class="handler-item" :class="{ 'disable': !canUndo }" v-tooltip="'Undo'" @click="undo()" />
      <IconNext class="handler-item" :class="{ 'disable': !canRedo }" v-tooltip="'Redo'" @click="redo()" />
      <div class="more">
        <Divider type="vertical" style="height: 20px;" />
        <IconComment class="handler-item" :class="{ 'active': showNotesPanel }" v-tooltip="'Comment'" @click="toggleNotesPanel()" />
        <IconMoveOne class="handler-item" :class="{ 'active': showSelectPanel }" v-tooltip="'Select Panel'" @click="toggleSelectPanel()" />
        <IconSearch class="handler-item" :class="{ 'active': showSearchPanel }" v-tooltip="'Find/Replace'" @click="toggleSraechPanel()" />
      </div>
    </div>
    ...
    <div class="right-handler">
      ...
      <Popover trigger="click" v-model:value="canvasScaleVisible">
        <template #content>
          ...
          <PopoverMenuItem center @click="resetCanvas()">Fit Screen</PopoverMenuItem>
        </template>
        <span class="text">{{canvasScalePercentage}}</span>
      </Popover>
      <IconPlus class="handler-item viewport-size" @click="scaleCanvas('+')" />
      <IconFullScreen class="handler-item viewport-size-adaptation" v-tooltip="'Fit Screen'" @click="resetCanvas()" />
    </div>
