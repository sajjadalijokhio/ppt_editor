      ...
      <div class="notes">
        <div class="note" :class="{ 'active': activeNoteId === note.id }" v-for="note in notes" :key="note.id" @click="handleClickNote(note)">
          ...
          <div class="note-reply" v-if="replyNoteId === note.id">
            <TextArea :padding="6" v-model:value="replyContent" placeholder="Enter reply content" :rows="1" />
        ...
        <div class="empty" v-if="!notes.length">No notes on this page yet</div>
      </div>
      <div class="send">
        <TextArea 
          ref="textAreaRef"
          ...
          :padding="6"
          :placeholder="`Add a note (for ${handleElementId ? 'selected element' : 'current slide' })`"
          :rows="2"
          @focus="replyNoteId = ''; activeNoteId = ''"
        />
        <div class="footer">
          <Button class="btn" v-tooltip="'Clear notes on this page'" style="flex: 1" @click="clear()"><IconDelete /></Button>
