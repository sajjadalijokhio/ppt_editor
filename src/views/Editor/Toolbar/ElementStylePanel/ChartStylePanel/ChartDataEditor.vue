...
const getTableData = () => {
  ...
  // 第一行为系列名，first row为项目名，solid际数据从第二行第二列start
...
// customizePaste事件（尝试读取剪贴板中的表格数据）
...
const changeSelectRange = (e: MouseEvent) => {
  ...
  document.onmouseup = e => {
    ...
    // 拖拽结束时，scope超过格子一半自动扩大到下一格（如拖动到一格半多的Location，会自动扩展到两格，横竖都同理）
