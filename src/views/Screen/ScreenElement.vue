...
// 判断元素是否需要等待执行入场animation：等待执行入场的元素需要先隐藏
const needWaitAnimation = computed(() => {
  // 该元素在本页animation序列中的Location
  ...
  // 若该元素已执行过animation，都无须隐藏
  // 具体来说：若已执行的最后一个animation为入场，显然无须隐藏；若已执行的最后一个animation为退场，由于保留了退场animation结束状态，也无需额外隐藏
  if (elementIndexInAnimation < props.animationIndex) return false

  // 若该元素未执行过animation，获取其将要执行的第一个animation
  // 若将要执行的第一个animation为入场，则需要隐藏，否则无须隐藏
